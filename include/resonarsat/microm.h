 ...
 *
 * THE p-VALUE IS EXACT ONLY WHEN THE TWO SAMPLES ARE INDEPENDENT, AND THAT IS A
 * PRECONDITION ON THE PAIRING, NOT A PROPERTY OF THIS FUNCTION. It holds when
 * the twin is a SEPARATE OBSERVATION of the same scene -- a second acquisition,
 * whose noise realisation differs. It does NOT hold for a twin generated by a
 * deterministic simulator at the same seed: measured, `sim_cphd` at one seed
 * produces a BIT-IDENTICAL file, so under H0 the ratio is exactly 1 on every
 * window rather than F(2,2)-distributed. In that case the p-value is grossly
 * CONSERVATIVE -- there is no noise floor at all and any r > 1 is attributable
 * to the motion -- and the LLR should be read as a signal measure with the
 * p ignored. The caller can detect the case: a twin sharing the noise
 * realisation gives r exactly 1 wherever nothing moved.
 *
 * Under independence the two samples are i.i.d. exponential, so their ratio is
 * F(2,2) and
 *
 *     P(r > x) = 1 / (1 + x)
 *
