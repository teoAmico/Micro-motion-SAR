...
static int rs_cmd_focus(int argc, char **argv)
{
    ...
    if (!in || !out) {
        printf("usage: resonarsat focus --cphd FILE --out FILE.png\n"
               ...
               "\n"
               "--rbins caps the SAMPLES READ PER PULSE and --max-pulses caps how\n"
               "many pulses are READ, not merely used. A large collect will not fit\n"
               "in memory otherwise: --pulse-count limits focusing but the whole\n"
               "collect is still loaded first.\n"
               "\n"
               "WHAT --rbins CUTS DEPENDS ON THE PRODUCT'S DomainType, and the two\n"
               "are not interchangeable. In a TIME-domain product the samples are\n"
               "delay bins, so it selects a near-range window and costs only scene\n"
               "extent. In an FX-domain product -- which is what Capella and Umbra\n"
               "ship -- the samples are TRANSMITTED FREQUENCIES, and the cut is\n"
               "applied BEFORE the inverse transform, so it narrows the BANDWIDTH\n"
               "and coarsens range resolution in proportion. Keeping 4000 of 21343\n"
               "FX samples is a 5.3x loss of range resolution, not a smaller scene.\n"
               "\n"
