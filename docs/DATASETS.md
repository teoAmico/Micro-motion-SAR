...
| **Umbra** | 4048 CPHD, but median dwell 3.50 s | Older task folders can carry `*_CPHD.cphd` beside GEC/SICD/SIDD. Do not trust an empty STAC asset as proof: a 2025 Kourou item advertises `MM.cphd`, but its anonymous S3 folder contains only GEC, SICD and SIDD. 81 named sites plus 1314 ad-hoc collects. Real CPHD declares `SGN = -1`, ships `CF8`, and omits `AmpSF`. | **Reads correctly, verified against the vendor's own GEC** (item 36). No override needed — its honest `SGN` produces the same transform Capella's override does. Focused a Panama Canal collect; not mirrored. Never run through the measurement chain. **DISQUALIFIED ON DWELL** (item 63): 4048 products sampled give median 3.50 s, 0.4% at >=15 s, none at >=30 s, so `df` = 0.29 Hz — coarser than most of the target band. Built for resolution and revisit, not staring. |
| **MOTION ABOVE THE FLOOR** | none found (item 61) | All 315 synchronised hits screened for RMS displacement in 0.03–3 Hz. 52 above the 5.5 µm floor raw; **0 survive** channel-type, chronically-high-station and neighbour auditing. No open Capella collect has independently confirmed motion this radar could have seen. Next: start from the earthquake catalogue, not from collects. |
| **SYNCHRONISED SENSORS** | 315 found (item 59) | `runs/screens/sensor-join/measurement_hits.csv` — FDSN instruments recording inside a Capella spotlight footprint during the aperture. Best: **Oroville Dam** `BK.ORV`, 30.4 s dwell, 32 GB; **Kilauea** with 40 repeat collects and 6+ stations each; **Etna** with 4 INGV stations. None downloaded. |
| **REQUIREMENT** | spotlight or dwell only | Per-target observation time must be SECONDS. In stripmap a point is lit for `lambda*R/(2*rho*V)` regardless of collect length — 0.71 s on Capella's 17 s KSTL stripmap, giving `df` = 1.4 Hz against a 0.3–3 Hz target band. Check the mode first; it disqualifies outright and costs nothing (item 58). |
