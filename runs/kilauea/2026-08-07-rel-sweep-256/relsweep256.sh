#!/bin/zsh
# Item 106: does the brightness transition move with look count?
...
O=$D/relsweep256_results.txt
echo "collect rel reported_hz prominence target_floor_mm phase_sd scene_median_mm" > $O
...
for S in CAPELLA_C10_SP_CPHD_HH_20240609091921_20240609091951 \
  ...
  # the uninjected control at this look count, so H3 has its reference
  ./build/micromotion mmotion --cphd "$DEST/$S.cphd" --estimator phase \
      ...
      --out $D/r2_${S}_ctl > $D/r2_${S}_ctl.log 2>&1
  python3 - "$D/r2_${S}_ctl.log" "$S" "ctl" >> $O <<'PY'
import re,sys
...
print("%s %s %s %s %s %s %s"%(sys.argv[2],sys.argv[3],m.group(1) if m else '-',
  m.group(2) if m else '-', f.group(1) if f else '-', f.group(2) if f else '-',
  s.group(1) if s else '-'))
PY
  for rel in 20 10 5 2 1; do
    ./build/micromotion mmotion --cphd "$DEST/$S.cphd" --estimator phase \
        ...
        --out $D/r2_${S}_$rel > $D/r2_${S}_$rel.log 2>&1
    python3 - "$D/r2_${S}_$rel.log" "$S" "$rel" >> $O <<'PY'
import re,sys
t=open(sys.argv[1]).read()
m=re.search(r'strongest peak in window \d+: ([0-9.]+) Hz, prominence ([0-9.]+)',t)
f=re.search(r'strongest-prominence window \d+: ([0-9.]+) mm \(phase sd ([0-9.]+) rad\)',t)
