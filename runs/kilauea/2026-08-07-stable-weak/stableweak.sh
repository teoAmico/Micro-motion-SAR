#!/bin/zsh
# Item 108: the stabilization test on a WEAK, LOCALISED target on REAL clutter.
# Pre-registered at 25f7351. Target 20 m off the grid origin, amplitudes
# bracketing item 103's 0.13-0.26 mm competition floor.
...
for S in CAPELLA_C10_SP_CPHD_HH_20240609091921_20240609091951 \
  ...
  for amp in 0.00 0.13 0.26 0.53; do
    if [ "$amp" = "0.00" ]; then INJ=(); else
      INJ=(--inject-wave "$D/naples_wave/seg02.txt,$RATE,$amp,20" --inject-at 20,20); fi
    ./build/micromotion mmotion --cphd "$DEST/$S.cphd" --estimator phase \
        ...
        --out $D/sw_${S}_${amp}_256 > $D/sw_${S}_${amp}_256.log 2>&1
    ./build/micromotion mmotion --cphd "$DEST/$S.cphd" --estimator phase \
        --n 128 --overlap 0 --size 256 --cell 0.5 --win 32 --upsample 200 \
        --coherence 0 --max-pulses $MP "${INJ[@]}" \
        --stable $D/sw_${S}_${amp}_256_windows.csv \
        --out $D/sw_${S}_${amp}_128 > $D/sw_${S}_${amp}_128.log 2>&1
    python3 - "$D/sw_${S}_${amp}_128.log" "$S" "$amp" >> $O <<'PY'
import re,sys
