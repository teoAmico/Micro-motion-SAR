...
static double g_lp_freq, g_lp_ratio, g_ps_freq; static int g_ps_ok;
...
int main(void)
{
    ...
    for (int L = 0; L < 4; L++) {
        ...
        printf("%10s | static in band %d/3        | static in band %d/3        |"
               " PS: answered %d/5 slope %+.3f rms %.4f, static answers %d/3 (%d in band)\n",
               "", in_band, l_in_band, np, pslope, prms, ps_ans, ps_in_band);
    }
